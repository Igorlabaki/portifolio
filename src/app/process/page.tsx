...
export default function ProcessPage() {
  ...
  return (
    ...
    <div className="min-h-screen h-full bg-trasparent pr-[100px] relative">
      ...
      <motion.div className=" h-[100px] w-10 absolute bottom-[-100px] left-[20px] bg-custom-black z-50" initial={{opacity:0}} animate={{opacity:1}} transition={{duration:1, delay:1}}/>
        <AnimatedVerticalComponent className="relative">
       ...
       <div className={`absolute top-[150px] left-[290px] large:left-[326px] leading-[154.78px] font-[700] z-50 flex flex-col ${helveltica.className}  ${bannerAnimation ? "text-custom-orange" : "text-custom-white"} duration-700 text-[135px] large:text-[170px]`}>
              <AnimatedTitleComponent  className=" z-50" title="SPECIFY" animation={bannerAnimation}/>
              <AnimatedTitleComponent  delay={0.4} className="  z-50" title="DEVELOP" animation={bannerAnimation}/>
              <AnimatedTitleComponent  delay={0.8} className="  z-50" title="& DELIVERY" animation={bannerAnimation}/>
          </div>

      </div>
      <div className="w-[564px] ml-[564px] flex flex-col mt-[50px] justify-start items-start">
        <motion.hr className='h-[2px]  w-[768px] flex text-custom-white' initial={{width:0}}  animate={{width: 768}} transition={{duration:1, delay: 0.8}}/>
        <motion.p className="text-custom-yellow w-full  mt-[10px] text-[12px]" initial={{opacity:0}}  animate={{opacity: 1}} transition={{duration:0.5, delay:1.2}}>In-depth Expertise, Idea Generation, and Extensive Knowledge in Visual Solutions Through Design</motion.p>
      </div>
      <div className="pl-[100px] mt-[90px] flex flex-col  leading-[16px] ">
        <div className="flex large:flex-col relative">
          <div className="flex large:flex-col">
            <div className=" w-[516px] text-[12px] flex large:flex-row large:gap-x-[20px] flex-col space-x-[135px] large:space-x-0 gap-y-[73px] large:gap-y-0">
          ...
          <div className="min-h-[150px] flex  mt-[80px] gapx-[20px]">
            ...
            <div className=" leading-[16px] text-[12px] flex large:flex-row large:gap-x-[20px] flex-col space-x-[135px] large:space-x-0 gap-y-[73px] large:gap-y-0">
              ...
              <AnimatedVerticalComponent   duration={1.4} className="flex flex-col justify-start items-start text-justify gap-y-[27px] w-[290px]">
                ...
                <p className="text-white leading-[16px]">Detailed design plans are put into action. It involves executing the project according to the established guidelines, often including coding, manufacturing, or construction, depending on the type of project. This phase aims to bring the project to life and achieve its intended outcomes.</p>
              </AnimatedVerticalComponent>
            </div>
          </div>
        </div>
